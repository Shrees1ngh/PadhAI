    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm sm:text-sm text-slate-400 mt-1">
        ...
        {onContinueLearning && (
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold text-sm shadow-lg shadow-indigo-600/30 transition-all flex items-center space-x-2 shrink-0 self-start sm:self-auto"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-3xl p-5 bg-[#0d1322] border border-white/10 flex items-center space-x-4 shadow-xl">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-400 font-medium">Curriculum Progress</p>
          </div>
        </div>
        ...
        <div className="rounded-3xl p-5 bg-[#0d1322] border border-white/10 flex items-center space-x-4 shadow-xl">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-400 font-medium">Lessons Finished</p>
          </div>
        </div>
        ...
        <div className="rounded-3xl p-5 bg-[#0d1322] border border-white/10 flex items-center space-x-4 shadow-xl">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-400 font-medium">Quiz Questions Solved</p>
          </div>
        </div>
        ...
        <div className="rounded-3xl p-5 bg-[#0d1322] border border-white/10 flex items-center space-x-4 shadow-xl">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-400 font-medium">Learning Streak 🔥</p>
          </div>
        </div>
        ...
        {courses.length === 0 ? (
          <div className="py-12 text-center rounded-2xl bg-[#080c14] border border-white/5 space-y-3">
            ...
            {onContinueLearning && (
              <button
                onClick={onContinueLearning}
                className="mt-2 px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-sm font-bold transition-all inline-flex items-center space-x-1.5"
              >
                ...
                >
                  <div className="space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <span className="text-sm font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-indigo-500/10 text-indigo-300 border border-indigo-500/20">
                          {c.category || 'Course'}
                        </span>
                  ...
                  <div className="pt-2 border-t border-white/5 flex items-center justify-end">
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-xl bg-white/5 hover:bg-indigo-600 text-slate-200 hover:text-white text-sm font-bold transition-all flex items-center space-x-1.5"
                    >
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="rounded-3xl p-6 sm:p-8 bg-[#0d1322] border border-white/10 shadow-xl space-y-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={`text-sm font-bold px-2.5 py-1 rounded-full border ${retentionColorClass}`}>
                {retentionLabel}
              </span>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-slate-400">Calculated from lesson completions and quiz accuracy</p>
                </div>
              </div>

              {/* Strong vs Needs Revision breakdown */}
              <div className="space-y-3 bg-[#080c14] p-4 rounded-2xl border border-white/5 text-sm">
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1.5">
                    {strongTopics.length > 0 ? strongTopics.map((topic, i) => (
                      <span key={i} className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-300 border border-emerald-500/20 text-sm font-semibold flex items-center space-x-1">
                        <Check className="w-2.5 h-2.5" />
                        <span className="truncate max-w-[140px]">{topic}</span>
                      </span>
                    )) : (
                      <span className="text-slate-500 italic text-sm">Pass quizzes (score ≥75%) to record strong topics</span>
                    )}
                  </div>
                </div>
                ...
                <div className="border-t border-white/5 pt-2">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-1.5">
                    {weakTopics.length > 0 ? weakTopics.map((topic, i) => (
                      <span key={i} className="px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-300 border border-amber-500/20 text-sm font-semibold flex items-center space-x-1">
                        <AlertTriangle className="w-2.5 h-2.5" />
                        <span className="truncate max-w-[140px]">{topic}</span>
                      </span>
                    )) : (
                      <span className="text-slate-500 italic text-sm">No revision flags detected</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="p-4 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 flex items-center space-x-3 text-sm">
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="rounded-3xl p-6 sm:p-8 bg-[#0d1322] border border-white/10 shadow-xl space-y-5">
            ...
            <div className="flex items-center justify-between border-b border-white/5 pb-3">
              ...
              <span className="text-sm font-semibold text-slate-400 flex items-center space-x-1">
            ...
            <div className="space-y-2.5">
              {goals.map((g) => (
                <div
                  ...
                  className={`p-3 rounded-2xl border cursor-pointer transition-all flex items-center space-x-3 text-sm select-none ${
                    g.done
            ...
            {onContinueLearning && (
              <button
                onClick={onContinueLearning}
                className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold text-sm shadow-xl shadow-indigo-600/30 transition-all flex items-center justify-center space-x-2 mt-4"
              >
