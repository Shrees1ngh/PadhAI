...
const Shuffle = forwardRef(({
  ...
  // Expose imperative trigger to parent via ref
    ...
    () => {
      ...
      const build = () => {
        ...
        try {
          ...
          // Fallback if SplitText cannot process DOM directly
          el.innerHTML = text.split('').map(c => `<span class="shuffle-char">${c === ' ' ? '&nbsp;' : c}</span>`).join('');
          splitRef.current = { chars: Array.from(el.querySelectorAll('.shuffle-char')), revert: () => { el.textContent = text; } };
        }
