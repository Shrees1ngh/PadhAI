          ...
          <div className="nav-left">
            <button onClick={() => handleNav('/')} className="nav-logo" aria-label="Go to home">
              <div className="relative group flex items-center gap-2.5">
                <div className="w-9 h-9 rounded-2xl overflow-hidden shadow-lg shadow-cyan-500/20 shrink-0 border border-cyan-500/30 bg-gradient-to-br from-white/10 via-cyan-500/10 to-transparent flex items-center justify-center p-1.5 transition-transform hover:scale-105">
                  <img 
                    src="/logo.svg" 
                    alt="PadhAI Logo" 
            ...
            {!isAuthenticated || !user ? (
              ...
              <div className="relative group">
                <button className="flex items-center gap-2.5 px-3.5 h-10 rounded-full bg-[#080c14] border border-white/20 hover:bg-[#0e1422] hover:border-white/30 transition-all text-xs font-bold text-white shadow-xl cursor-pointer">
                  ...
                  <span className="font-bold text-white tracking-wide text-xs max-w-[100px] truncate">{user.name ? user.name.split(' ')[0] : 'Account'}</span>
                ...
                <div className="absolute right-0 mt-2 w-52 rounded-2xl bg-[#080c14]/95 backdrop-blur-2xl border border-white/20 shadow-[0_10px_40px_rgba(0,0,0,0.9)] p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-1 group-hover:translate-y-0 z-50">
