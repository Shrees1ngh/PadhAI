...
const renderIcon = (icon) => {
  ...
  if (typeof icon === 'function') {
    const IconComp = icon;
    return <IconComp size={15} strokeWidth={1.8} />;
  }
  return null;
};
...
export default function BranchedMenu({
  ...
  const active = activeValue !== null && activeValue !== undefined ? activeValue : internalActive;
        ...
        return (
          <div key={item.value ?? item.label} className="branched-menu__section" data-open={isOpen ? '' : undefined}>
            ...
            >
              ...
              {kids && (
                <span className={`text-[10px] text-slate-500 transition-transform duration-200 ${isOpen ? 'rotate-90 text-cyan-400' : ''}`}>
                  ▶
                </span>
              )}
            </button>
