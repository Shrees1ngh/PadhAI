...
import { callOllama, isOllamaAvailable } from "./ollama.service.js";
...
/**
 * ============================================================
 *  UNIFIED AI ROUTER — callAI()
 *  Automatically selects Ollama or Gemini based on AI_PROVIDER.
 *
 *  AI_PROVIDER="ollama" (default) → use Ollama, fall back to Gemini if down
 *  AI_PROVIDER="gemini"           → use Gemini only
 *  AI_PROVIDER="auto"             → same as "ollama"
 *
 *  All existing services call callGemini() which is now an alias for callAI().
 * ============================================================
 */
export const callAI = async (options) => {
  ...
  // --- Ollama mode (with Gemini fallback) ---
  try {
    const result = await callOllama({
      ...
      model: options.model,       // uses OLLAMA_MODEL env if not specified
      jsonMode: options.jsonMode ?? true,
    });
    return result;
  } catch (ollamaError) {
    const isUnavailable =
      ollamaError.code === "OLLAMA_UNAVAILABLE" ||
      ollamaError.code === "OLLAMA_MODEL_NOT_FOUND" ||
      ollamaError.status === 503;

    if (isUnavailable) {
      const geminiKey = resolveApiKey(options.apiKey);

      if (geminiKey) {
        console.warn(
          `[AI Router] Ollama unavailable (${ollamaError.message}). Falling back to Gemini...`
        );
        return callGemini(options);
      }
    ...
    // Ollama responded but output was bad → don't fall back to Gemini, surface the error
    throw ollamaError;
  }
};

// Backward-compatible alias — all existing services (topic.service.js, etc.) keep working unchanged
export { callAI as callGeminiOrOllama };
